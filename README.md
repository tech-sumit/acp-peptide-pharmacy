...
- All MPP charges in this demo are **sandbox** amounts (currently **$0.01 USD** per paid MCP request).
...
1. `POST /mcp` without a valid `Authorization: Payment ...` credential returns **402** with a `WWW-Authenticate: Payment` challenge (RFC 9457 problem body).
2. After the client pays (Stripe test mode + SPT) and retries with the credential, the Worker runs MCP and may attach a **`Payment-Receipt`** header on success.
...
Unpaid request (expect `402` when MPP is configured):
...
Paid retry requires an MPP-aware client (for example [`npx mppx`](https://www.npmjs.com/package/mppx) against a local dev server, or a small app using Stripe Elements to mint an SPT per Stripe’s docs). See `mcp-config/README.md`.
